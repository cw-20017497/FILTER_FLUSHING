...
#if 0
#define EOL_LOAD_MODE_1 0x01        // 검사 모드 진입 
#define EOL_LOAD_MODE_2 0x02        // 자동화 검사 시작
static void ParserStart(U8 *buf, I16 len)
{
    U8 mu8TestMode;
...
static I16	MakePkt_Setting( U8 *buf, U16 mu16PktType )
{
    ...
    mi16Len += SPRINTF( (char __FAR *)&buf[ mi16Len ], (const char __FAR *)"%02X", 0 );                   // UNUSED - 고도 모드
    mi16Len += SPRINTF( (char __FAR *)&buf[ mi16Len ], (const char __FAR *)"%02X", COUNTRY_CODE_KOR );    // 국가코드
...
static U16 ConvEolTemp(U8 mu8AdcId, TEMP_T mTemp)
{
    ...
    else
    {
        ...
        if( mTemp > 0.0f )
        {
            mu16Temp = (U16)( mTemp * 10.0f );  // 소수점 첫째 자리까지 표현
        }
        else
        {
            // 음수인 경우... 1xxx로 표현...
            mTemp = -mTemp;                     // 부호 변경
            mu16Temp = (U16)( mTemp * 10.0f );  // 소숫점 첫째 자리까지 표현
            mu16Temp += 1000;
        }
    }
