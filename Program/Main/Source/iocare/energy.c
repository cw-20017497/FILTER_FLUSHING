...
#define CONFIG_USE_EEPROM_ENERGY    0       // 소비전력 eeprom 저장 여부(0: 미사용, 1:사용)
...
//#define STAND_BY_BACK_LIGHT_ON_WH   36UL      // 0.036w   @1min  ( 대기 모드는 사용량이 작아서 1분이 한 번 적산 )
//#define STAND_BY_BACK_LIGHT_OFF_WH  19UL      // 0.019w   @1min  ( 대기 모드는 사용량이 작아서 1분에 한 번 적산 )

//#define STAND_BY_BACK_LIGHT_ON_WH   37UL      // 0.036w   @1min  ( 대기 모드는 사용량이 작아서 1분이 한 번 적산 )
//#define STAND_BY_BACK_LIGHT_OFF_WH  20UL      // 0.019w   @1min  ( 대기 모드는 사용량이 작아서 1분에 한 번 적산 )

//#define STAND_BY_BACK_LIGHT_ON_WH   72UL      // 0.036w x 2  @1min  ( 대기 모드는 사용량이 작아서 1분이 한 번 적산 )
//#define STAND_BY_BACK_LIGHT_OFF_WH  57UL      // 0.019w x 3  @1min  ( 대기 모드는 사용량이 작아서 1분이 한 번 적산 )
...
U32 GetEnergyRateMonth(U8 mu8Month)
{
    ...
    if( mu8Month >= 1 && mu8Month <= 12 )
    {
        ...
        //mu32Watt /= 1000;   // 소숫점 3자리 버림
        mu32Watt /= 1000;   // kw 변환

        mu32Rate = mu32Watt * RATE_1KWH;    // 전기료 계산
        mu32Rate /= 10000UL;                  // 소숫점 버림 
    }
...
static U16 CalcValves(void)
{
    ...
    if( IsOpenValve( VALVE_ROOM_OUT ) == TRUE )     { mu16Sum += VALVE_WH; }
    ...
    if( IsOpenValve( VALVE_COLD_OUT ) == TRUE )     { mu16Sum += VALVE_WH; }
...
void ProcessEnergy(void)
{
    ...
    // Month가 변경되면, 새로운 Month의 watt는 초기화
    GetRtcTime( &mTime );
    if( mu8Init == FALSE )
    {
        mu8Init = TRUE;
